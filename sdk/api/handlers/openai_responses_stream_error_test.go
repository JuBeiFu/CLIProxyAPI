...
func TestBuildOpenAIResponsesStreamErrorChunkMaps429ToServiceUnavailable(t *testing.T) {
	chunk := BuildOpenAIResponsesStreamErrorChunk(
		http.StatusTooManyRequests,
		`{"error":{"code":"model_cooldown","message":"All credentials for model gpt-5.4 are cooling down"}}`,
		0,
	)
	var payload map[string]any
	if err := json.Unmarshal(chunk, &payload); err != nil {
	...
	if payload["code"] != "service_unavailable" {
		t.Fatalf("code = %v, want %q", payload["code"], "service_unavailable")
	}
}

func TestBuildOpenAIResponsesStreamErrorChunkMapsCurrentModelUnavailable429ToServiceUnavailable(t *testing.T) {
	chunk := BuildOpenAIResponsesStreamErrorChunk(
		http.StatusTooManyRequests,
		`{"error":{"message":"The requested model is currently unavailable. Please switch model.","type":"rate_limit_exceeded"}}`,
		0,
	)
	var payload map[string]any
	if err := json.Unmarshal(chunk, &payload); err != nil {
