...
func TestImagesGenerationsRouteUsesAuthMiddleware(t *testing.T) {
	server := newTestServer(t)

	req := httptest.NewRequest(http.MethodPost, "/v1/images/generations", strings.NewReader(`{"model":"gpt-image-1","prompt":"draw a cat"}`))
...
func TestImagesEditsRouteUsesAuthMiddleware(t *testing.T) {
	server := newTestServer(t)

	req := httptest.NewRequest(http.MethodPost, "/v1/images/edits", strings.NewReader(`{"model":"gpt-image-2","prompt":"edit a cat"}`))
	req.Header.Set("Content-Type", "application/json")
...
func TestImagesVariationsRouteUsesAuthMiddleware(t *testing.T) {
	server := newTestServer(t)

	req := httptest.NewRequest(http.MethodPost, "/v1/images/variations", strings.NewReader(`{"model":"dall-e-2"}`))
	req.Header.Set("Content-Type", "application/json")
