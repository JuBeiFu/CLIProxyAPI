...
func TestCodexExecutorExecuteStreamSynthesizesCompletionAfterFunctionCallDoneEOF(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_function_tool_partial\",\"created_at\":1775555723,\"model\":\"gpt-5.5\"}}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.added\",\"item\":{\"id\":\"fc_1\",\"type\":\"function_call\",\"call_id\":\"call_1\",\"name\":\"tool_search\"},\"output_index\":0}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.function_call_arguments.done\",\"item_id\":\"fc_1\",\"arguments\":\"{\\\"query\\\":\\\"hello\\\"}\"}\n\n"))
		if flusher, ok := w.(http.Flusher); ok {
			flusher.Flush()
		}
	}))
	...
	if !bytes.Contains(got.Bytes(), []byte(`"type":"function_call"`)) {
		t.Fatalf("expected synthesized completion function call output; got %s", got.String())
	}
	if !bytes.Contains(got.Bytes(), []byte(`\"query\":\"hello\"`)) {
		t.Fatalf("expected synthesized completion to retain function arguments; got %s", got.String())
	}
}

func TestCodexExecutorExecuteStreamErrorsAfterFunctionCallDeltaOnlyEOF(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_function_tool_delta\",\"created_at\":1775555723,\"model\":\"gpt-5.5\"}}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.added\",\"item\":{\"id\":\"fc_1\",\"type\":\"function_call\",\"call_id\":\"call_1\",\"name\":\"tool_search\"},\"output_index\":0}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.function_call_arguments.delta\",\"item_id\":\"fc_1\",\"delta\":\"{\\\"query\\\":\"}\n\n"))
		if flusher, ok := w.(http.Flusher); ok {
			flusher.Flush()
		}
	}))
	...
	if bytes.Contains(got.Bytes(), []byte("response.completed")) {
		t.Fatalf("partial function call arguments must not synthesize response.completed; got %s", got.String())
	}
	if gotErr == nil {
		t.Fatal("expected terminal stream error after partial function call arguments")
	}
}

func TestCodexExecutorExecuteStreamSynthesizesCompletionAfterCustomToolEOF(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_custom_tool_partial\",\"created_at\":1775555723,\"model\":\"gpt-5.5\"}}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.added\",\"item\":{\"id\":\"ctc_1\",\"type\":\"custom_tool_call\",\"call_id\":\"call_1\",\"name\":\"apply_patch\"},\"output_index\":0}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.custom_tool_call_input.done\",\"item_id\":\"ctc_1\",\"input\":\"*** Begin Patch\"}\n\n"))
		if flusher, ok := w.(http.Flusher); ok {
			flusher.Flush()
		}
	}))
	...
	if !bytes.Contains(got.Bytes(), []byte(`"type":"custom_tool_call"`)) {
		t.Fatalf("expected synthesized completion custom tool output; got %s", got.String())
	}
	if !bytes.Contains(got.Bytes(), []byte("*** Begin Patch")) {
		t.Fatalf("expected synthesized completion to retain custom tool input; got %s", got.String())
	}
}

func TestCodexExecutorExecuteStreamErrorsAfterCustomToolDeltaOnlyEOF(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_custom_tool_delta\",\"created_at\":1775555723,\"model\":\"gpt-5.5\"}}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.added\",\"item\":{\"id\":\"ctc_1\",\"type\":\"custom_tool_call\",\"call_id\":\"call_1\",\"name\":\"apply_patch\"},\"output_index\":0}\n\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.custom_tool_call_input.delta\",\"item_id\":\"ctc_1\",\"delta\":\"*** Begin\"}\n\n"))
		if flusher, ok := w.(http.Flusher); ok {
			flusher.Flush()
		}
	}))
	...
	if bytes.Contains(got.Bytes(), []byte("response.completed")) {
		t.Fatalf("partial custom tool input must not synthesize response.completed; got %s", got.String())
	}
	if gotErr == nil {
		t.Fatal("expected terminal stream error after partial custom tool input")
	}
}

func TestCodexExecutorExecuteStreamErrorsAfterReasoningOnlyEOF(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
	...
	if bytes.Contains(got.Bytes(), []byte("response.completed")) {
		t.Fatalf("reasoning-only stream must not synthesize response.completed; got %s", got.String())
	}
	if !bytes.Contains(got.Bytes(), []byte("thinking only")) {
		t.Fatalf("expected reasoning text before failure; got %s", got.String())
	}
	if gotErr == nil {
		t.Fatal("expected terminal stream error when only reasoning arrives before EOF")
	}
