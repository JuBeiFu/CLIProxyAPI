...
func TestConvertOpenAIRequestToCodex_CompletesRequiredInResponseFormatSchemaDefs(t *testing.T) {
	input := []byte(`{
		"model": "gpt-4o",
		"messages": [
	...
	out := ConvertOpenAIRequestToCodex("gpt-4o", input, true)
	required := gjson.GetBytes(out, `text.format.schema.$defs.result.required`).Array()
	if len(required) != 2 {
		t.Fatalf("expected 2 required fields under $defs.result, got %d: %s", len(required), gjson.GetBytes(out, "text.format.schema").Raw)
	}
	if !gjson.GetBytes(out, `text.format.schema.$defs.result.required.#(=="foo")`).Exists() {
		t.Fatalf("expected foo to stay required: %s", gjson.GetBytes(out, "text.format.schema").Raw)
	}
	if !gjson.GetBytes(out, `text.format.schema.$defs.result.required.#(=="bar")`).Exists() {
		t.Fatalf("expected bar to be auto-added to required: %s", gjson.GetBytes(out, "text.format.schema").Raw)
	}
}

func TestConvertOpenAIRequestToCodex_CompletesRequiredInResponseFormatSchemaDefinitions(t *testing.T) {
	input := []byte(`{
		"model": "gpt-4o",
		"messages": [
			{"role": "user", "content": "Return structured output"}
	...
	out := ConvertOpenAIRequestToCodex("gpt-4o", input, true)
	required := gjson.GetBytes(out, `text.format.schema.definitions.payload.required`).Array()
	if len(required) != 2 {
		t.Fatalf("expected 2 required fields under definitions.payload, got %d: %s", len(required), gjson.GetBytes(out, "text.format.schema").Raw)
	}
	if !gjson.GetBytes(out, `text.format.schema.definitions.payload.required.#(=="alpha")`).Exists() {
		t.Fatalf("expected alpha to be required: %s", gjson.GetBytes(out, "text.format.schema").Raw)
	}
	if !gjson.GetBytes(out, `text.format.schema.definitions.payload.required.#(=="beta")`).Exists() {
		t.Fatalf("expected beta to be required: %s", gjson.GetBytes(out, "text.format.schema").Raw)
	}
}
